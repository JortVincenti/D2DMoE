...
class VAR(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        debug_data = {
            ...
            "idx_Bl": [],
            "h_BChw": [],
            "logits_BlV": [],  # add others if desired
            "ratio": [],
            'next_token_map': [],
            'x': [],
            'cond_BD_or_gss': [],
        ...
        for si, pn in enumerate(self.patch_nums):  # si: i-th segment
            ...
            debug_data["cond_BD_or_gss"].append(cond_BD_or_gss.detach().cpu().clone())
            x = next_token_map
            block_output = []
            for b in self.blocks:
            ...
            debug_data["block_output"].append(block_output)
            debug_data["x"].append(x.detach().cpu().clone())
            logits_BlV = self.get_logits(x, cond_BD)
           

            t = cfg * ratio
            logits_BlV = (1 + t) * logits_BlV[:B] - t * logits_BlV[B:]
            debug_data["logits_BlV"].append(logits_BlV.detach().cpu().clone())

            idx_Bl = sample_with_top_k_top_p_(logits_BlV, rng=rng, top_k=top_k, top_p=top_p, num_samples=1)[:, :, 0]
            debug_data["idx_Bl"].append(idx_Bl.detach().cpu().clone())
            ...
            if si != self.num_stages_minus_1:
                ...
                debug_data["next_token_map"].append(next_token_map.detach().cpu().clone())
